...
3. For .NET postprocessors the editor is opened on the postprocessor folder itself, so what lies above it can fall out of view. Put the repository at the top of the whole workspace rather than inside one postprocessor, and leave a two-line `AGENTS.md` in the postprocessor folder pointing at the main one and at the notes. Offer a multi-root workspace file when the user wants both in front of them at once.
