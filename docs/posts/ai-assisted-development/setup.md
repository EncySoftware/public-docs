...
1. Download the tools bundle archive — its name starts with `posts-vscode-extensions-`. Its address is on the [External references and examples](xref:posts-external-references) page.
...
- **Local Markdown.** This documentation is published as a Markdown repository; its address is on the same [External references and examples](xref:posts-external-references) page. Clone it with `git clone --depth 1 <url>` and point the assistant at the `docs/posts` folder — as a second workspace folder, or by naming the path in the client's instruction file. Update it with `git pull`.
